...
void b2World::DrawParticleSystem(const b2ParticleSystem& system)
{
	...
	if (particleCount)
	{
		...
		const b2Vec2* velocityBuffer = system.GetVelocityBuffer();
		if (system.m_colorBuffer.data)
		{
			const b2ParticleColor* colorBuffer = system.GetColorBuffer();
			m_debugDraw->DrawParticles(positionBuffer, velocityBuffer, radius, colorBuffer, particleCount);
		}
		else
		{
			m_debugDraw->DrawParticles(positionBuffer, velocityBuffer, radius, NULL, particleCount);
		}
	}
}
